...
public static class UpdateInstaller
{
    ...
    public static void Launch(PreparedUpdate update, string installDirectory)
    {
        ...
        var runnerPath = Path.Combine(
            runnerDirectory,
            $"HelloDpi.Updater-{GetCurrentVersion()}-{Guid.NewGuid():N}.exe");
    ...
    private static async Task<int> RunApplyAsync(IReadOnlyList<string> arguments)
    {
        ...
        var installedExecutable = Path.Combine(installDirectory, "HelloDpi.exe");
        if (!File.Exists(installedExecutable))
        {
            throw new InvalidDataException("The selected installation directory is not a HelloDpi DPI installation.");
        }
        ...
        string stagingDirectory)
    {
        ...
        var expectedRoot = $"HelloDpi-MVP-{manifest.Version}-win-x64/";
        ...
        RequireStagedFile(files, "HelloDpi.exe");
        ...
        var executableVersionText = FileVersionInfo.GetVersionInfo(
            Path.Combine(stagingDirectory, "HelloDpi.exe")).ProductVersion;
        ...
        string backupDirectory)
    {
        var orderedFiles = relativeFiles
            .OrderBy(path => Path.GetFileName(path).Equals("HelloDpi.exe", StringComparison.OrdinalIgnoreCase))
        ...
        string? downloadPath)
    {
        ...
        if (fullRunnerPath.StartsWith(normalizedRunnerRoot, StringComparison.OrdinalIgnoreCase)
            && Path.GetFileName(fullRunnerPath).StartsWith("HelloDpi.Updater-", StringComparison.Ordinal))
        {
