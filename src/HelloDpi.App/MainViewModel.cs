...
public sealed class MainViewModel : INotifyPropertyChanged, IAsyncDisposable
{
    ...
    private async Task ConnectAsync()
    {
        ...
        StrategyText = "Discord, ChatGPT используют отдельный AI-маршрут; видео YouTube остаётся быстрым";
        ...
        DetailText = _tunnel.TelegramSetupPromptOpened
            ...
            ? "Discord, ChatGPT используют AI-маршрут; Telegram — Tor, видео YouTube идёт напрямую."
            ...
            service.Update(
                result.IsHealthy
                    ? viaTunnel
                        ? $"Туннель · {latency}"
                        : $"Доступен · {latency}"
                    : service.Kind is ServiceKind.Telegram or ServiceKind.ChatGPT
                        ? "Туннель недоступен"
                        : "Недоступен",
