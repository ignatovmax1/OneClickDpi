...
static async Task<int> RunAsync(string[] arguments)
{
    if (arguments is ["--vps-only"])
    {
        ...
        for (var attempt = 0; attempt < 2; attempt++)
        {
            ...
            if (attempt == 0)
            {
                await using var vpsProxy = new SelectiveHttpProxy(new HelloDpi.Core.SelectiveRouteMatcher(), aiSocksPort: vps.SocksPort, listenPort: 19181);
    ...
    var proxy = new SelectiveHttpProxy(
        new HelloDpi.Core.SelectiveRouteMatcher(),
