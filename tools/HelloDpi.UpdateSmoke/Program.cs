...
var apiJson = JsonSerializer.SerializeToUtf8Bytes(new
{
    ...
    assets = new object[]
    {
        Asset(1, "HelloDpi-update.json", manifestBytes.Length, releaseBase, assetApiBase, null),
        Asset(2, "HelloDpi-update.sig", Encoding.ASCII.GetByteCount(signatureText), releaseBase, assetApiBase, null),
...
var temporary = Path.Combine(Path.GetTempPath(), "HelloDpi.UpdateSmoke", Guid.NewGuid().ToString("N"));
...
try
{
        ...
        new FakeHandler(apiUri, apiJson, assets)))
    {
        ...
        if (!extracted.Contains("HelloDpi.exe", StringComparer.OrdinalIgnoreCase))
        {
    ...
    {
        var entry = archive.CreateEntry($"HelloDpi-MVP-{manifest.Version}-win-x64/../escape.txt");
